        ...
        <div>
            ...
            <nav class="px-4 py-6 space-y-2 text-sm font-medium text-gray-700">
                ...
                @if(auth()->user()->hasAnyRole(['administrador','restauranteadmin','mesero','cajero']))
                    ...
                    <!-- Facturas -->
                    <a href="{{ route('facturas.index', $activeRest) }}"
                       class="{{ request()->routeIs('facturas.*') ? 'bg-[#153958] text-white' : 'hover:bg-[#F2F2F2] text-[#153958]' }} flex items-center px-4 py-2 rounded-md transition"
                       @click="open = false">
                        <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
