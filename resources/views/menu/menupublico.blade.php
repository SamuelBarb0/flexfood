...
<style>
  /* Oculta elementos hasta que Alpine los muestre */
  [x-cloak] { display: none !important; }

  /* Loader a pantalla completa */
  .loader-backdrop {
    position: fixed; inset: 0; background: #0C3558;
...
{{-- Loader (se oculta cuando $store.boot.ready === true) --}}
<div x-data x-show="!$store.boot?.ready" x-cloak
...
<div class="max-w-7xl mx-auto px-4 py-8 pb-20"
    ...
    <div class="sticky top-0 z-40 bg-white py-3 mb-6 border-b shadow-sm">
      <div class="overflow-x-auto scrollbar-hide [scrollbar-width:none]">
        <div class="flex gap-3 px-4 snap-x snap-mandatory">
    ...
    {{-- Menú inferior fijo tipo app (UNA sola vez) --}}
...
<script>
  // Alpine store global para el estado de arranque
  ...
  // Espera a que carguen todas las imágenes del contenedor
  async function waitImages(container = document) {
  ...
  // Espera a que carguen las fuentes (si es soportado)
  function waitFonts() {
  ...
  // Dos frames para estabilizar sticky/fixed/CLS
  ...
  function menuCarrito() {
    ...
    return {
      ...
      async bootstrap() {
        try {
          await waitFonts();
          this.init();                 // tu init original
          await waitImages(document);  // espera imágenes
          await waitNextFrames(2);     // asienta el layout
        } finally {
          Alpine.store('boot').setReady();
          // Micro-nudge opcional para iOS
